...
const publicSharesService = new PublicSharesService();
...
export class PublicShareAdapter {
  static create = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const share = await publicSharesService.createShare(
      req.session.user_id!,
      collection_id,
  ...
  static update = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const share = await publicSharesService.updateShare(share_id, req.session.user_id!, req.body);
  ...
  static delete = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const share_id = req.params["share_id"] as string;
    if (!share_id) { ActionResponse.error(res, "share_id is required", 400); return; }
    await publicSharesService.deleteShare(share_id, req.session.user_id!);
  ...
  static getForCollection = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const share = await publicSharesService.getForCollection(collection_id, req.session.user_id!);
  ...
  static resolve = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const data = await publicSharesService.resolvePublicShare(token);
