  ...
  return (
    <div className="lp-glow-card lp-lift p-6 md:p-7">
      <div className="relative z-10">
        ...
        <div className="mt-5 text-[1.05rem] font-semibold text-white">{title}</div>
        <p className="mt-2 text-[13.5px] text-secondary_text leading-relaxed">{body}</p>
        {kbd && (
          <div className="mt-5 flex items-center gap-1.5 text-[11px] text-secondary_text font-mono">
            {kbd.map((k, i) => (
              <span key={i} className="border border-white/15 bg-white/[0.03] rounded px-1.5 py-0.5">
                {k}
              </span>
            ))}
          </div>
    ...
    <section id="features" className="relative py-20 md:py-28">
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-10">
        <div className="max-w-[720px]">
          <div className="text-xs tracking-widest uppercase text-primary/90 font-medium">Features</div>
          ...
          <p className="mt-4 text-secondary_text text-base max-w-[520px]">
            No more lost tabs or messy bookmark folders. Just a calm, fast, keyboard-first home for the web you care about.
          </p>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-3 text-[13px]">
          {secondaryFeatures.map(([title, desc], i) => (
            <div key={i} className="rounded-lg border border-white/10 bg-white/[0.02] p-4 hover:bg-white/[0.04] transition">
              <div className="text-white font-medium">{title}</div>
              <div className="mt-1 text-secondary_text text-[12.5px] leading-relaxed">{desc}</div>
            </div>
          ))}
        </div>
      </div>
