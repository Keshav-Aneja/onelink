...
export function NotificaitonContent({ className }: ContentProps) {
  return (
    <div className={cn(className, "w-100 p-4")}>
      <span className="w-full text-left text-xl  tracking-wide truncate font-semibold border-b-1 border-b-white/40 pb-2">
        Notifiations
      </span>
...
export function NotificationItem() {
  return (
    <div className="w-full flex flex-col gap-2 not-first:border-t-1 border-white/20 not-first:-mt-4 pt-4 last:-mb-4">
      <section className="relative">
        <h3 className="text-base font-medium">New Blog</h3>
        <p className="text-sm text-secondary_text h-16 w-full text-wrap truncate">
      ...
      <aside className="self-end relative bottom-8">
        <Button className="text-sm hover:bg-primary">Read more</Button>
      </aside>
    </div>
  );
}
