...
const SidebarButton = ({ item, active, setActive }: SidebarButtonProps) => {
  ...
  return (
    ...
    >
      ...
      <Icon />
      <div className="group-hover:opacity-100 opacity-0 pointer-events-none absolute top-1/2 -translate-y-1/2 left-18 text-nowrap text-sm bg-primary text-black border-1 border-black font-medium px-3 py-1 rounded-md transition-all duration-200 ease-linear">
        {label}
      </div>
    </button>
  );
