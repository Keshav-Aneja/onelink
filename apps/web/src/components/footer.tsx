...
const footerGroups = [
  {
    label: "Product",
    links: [
      { name: "Collections", href: paths.collections.root.getHref(), external: false },
  ...
  },
  {
    label: "Developer",
    links: [
      { name: "GitHub", href: "https://github.com/Keshav-Aneja/onelink", external: true },
      { name: "API Docs", href: "#", external: false },
      { name: "Creator's Site", href: "https://keshavaneja.in", external: true },
    ...
    <footer className="relative pt-16 pb-8 border-t border-white/10">
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-10">
        <div className="grid grid-cols-1 md:grid-cols-[1.5fr_3fr] gap-10">
          <div>
            <div className="flex items-center gap-2.5">
              <span className="relative inline-block w-8 h-8">
                ...
                <span className="absolute inset-0.75 rounded-[5px] bg-theme_primary_black flex items-center justify-center text-primary text-sm font-bold">
                  ◉
                </span>
              </span>
          ...
          <div className="grid grid-cols-3 gap-6">
            {footerGroups.map((group, i) => (
              <div key={i}>
                <div className="text-[10.5px] uppercase tracking-[0.18em] text-secondary_text">{group.label}</div>
        ...
        <div className="mt-12 border-t border-white/10 pt-5 text-center text-xs text-secondary_text">
          © 2025 OneLink · Made with <span className="text-primary">♥</span> by Keshav Aneja
        </div>
      </div>
    </footer>
  );
